...
export function ReactShader({
  ...
  const setupChannelRes = ({ width, height }: Texture, id: number) => {
    // @ts-expect-error TODO: Deal with this.
    uniformsRef.current.iChannelResolution.value[id * 3] =
      width * devicePixelRatio;
    // @ts-expect-error TODO: Deal with this.
    uniformsRef.current.iChannelResolution.value[id * 3 + 1] =
      ...
      for (const name of Object.keys(propUniforms)) {
        ...
        if (isMatrixType(type, value)) {
          ...
          tempObject.arraySize = `[${Math.floor(
            value.length / Number.parseInt(type.charAt(0))
          )}]`;
        }
  ...
  const setUniforms = (timestamp: number) => {
    ...
    if (uniformsRef.current.iFrame?.isNeeded) {
      ...
      gl.uniform1i(
        timeDeltaUniform,
        (uniformsRef.current.iFrame.value as number)++
      );
    }
